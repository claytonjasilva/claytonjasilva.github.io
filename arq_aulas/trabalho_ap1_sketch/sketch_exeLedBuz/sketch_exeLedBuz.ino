...
de instruções para controle de 3 LEDs e 1 buzzer via
teclado de membrana 4x4, utilizando a biblioteca Keypad.h.
...
void processarEstadoLoadEnderecoInicial() {
  ...
  Serial.println("Nesta etapa: LEDON, LEDOFF, BUZON, BUZOFF e HALT.");
}
...
void processarEstadoLoadOpcode() {
  ...
  if (!opcodePermitidoNaEtapaAtual(opcodeAtual)) {
    Serial.println("ERRO: nesta etapa sao permitidos apenas LEDON, LEDOFF, BUZON, BUZOFF e HALT.");
    return;
  }
