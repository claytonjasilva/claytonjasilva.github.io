...
de instruções para controle de 3 LEDs, 1 buzzer e ACC de
16 bits, via teclado de membrana 4x4, utilizando Keypad.h.
...
2. MODO RUN:
   - após o carregamento, o sistema solicita o endereço da
     primeira instrução
   - esse endereço é carregado em PC
   - cada tecla '*' executa uma instrução da memória

3. ENCERRAMENTO:
   - após HALT em execução, a aplicação é encerrada
   - não aceita novas operações nesta versão

Nesta versão, o programa admite:
- READ
- LOADK
- ADDK
- SUBK
...
// =======================================================
// CONFIGURAÇÃO DOS LEDS, BUZZER E SENSOR
// =======================================================
...
void executarREAD();
void executarLOADK(uint16_t operando);
void executarADDK(uint16_t operando);
void executarSUBK(uint16_t operando);
...
long lerDistanciaCM();
uint16_t lerDistanciaEm2Segundos();
...
void exibirEstadoSaidas();
void exibirACC();
void exibirHexadecimal4Digitos(uint16_t valor);
...
void exibirMensagemInicial() {
  ...
  Serial.println("2  -> LOADK");
  Serial.println("3  -> ADDK");
  Serial.println("4  -> SUBK");
  Serial.println("6  -> LEDON");
  Serial.println("7  -> LEDOFF");
  Serial.println("8  -> BUZON");
  Serial.println("9  -> BUZOFF");
  Serial.println("15 -> HALT");
  Serial.println();
  Serial.println("LED 1 -> pino 2");
  Serial.println("LED 2 -> pino 3");
  Serial.println("LED 3 -> pino 4");
  Serial.println("BUZZER -> pino 5");
  Serial.println("TRIG -> pino 6");
  Serial.println("ECHO -> pino 7");
  Serial.println("ACC -> 16 bits");
  Serial.println();
...
void processarEstadoLoadEnderecoInicial() {
  ...
  Serial.println("Nesta etapa: READ, LOADK, ADDK, SUBK, LEDON, LEDOFF, BUZON, BUZOFF e HALT.");
}
...
void processarEstadoLoadOpcode() {
  ...
  if (!opcodePermitidoNaEtapaAtual(opcodeAtual)) {
    Serial.println("ERRO: opcode nao permitido nesta etapa.");
    return;
  }
  ...
  if (instrucaoExigeOperando(opcodeAtual)) {
    ...
    if (opcodeAtual == 6 || opcodeAtual == 7) {
...
void processarEstadoLoadOperando() {
  ...
  if ((opcodeAtual == 6 || opcodeAtual == 7) && !operandoLEDValido(operando)) {
...
long lerDistanciaCM() {
  ...
  // leitura do echo
  duracao = pulseIn(PINO_ECHO, HIGH, 30000UL);

  if (duracao == 0) {
    return -1;
  }

  // conversao aproximada em cm para HC-SR04
...
uint16_t lerDistanciaEm2Segundos() {
  ...
  long distanciaAtual;
  long ultimaLeituraValida = -1;

  instanteInicial = millis();

  while (millis() - instanteInicial < 2000UL) {
    distanciaAtual = lerDistanciaCM();

    if (distanciaAtual >= 0) {
      ultimaLeituraValida = distanciaAtual;
    }

    delay(60);
  }

  if (ultimaLeituraValida < 0) {
    return 0;
  }

  return (uint16_t) ultimaLeituraValida;
}

// =======================================================
// EXECUÇÃO DAS INSTRUÇÕES DE ACC
// =======================================================

void executarREAD() {
  Serial.println("EXEC   -> READ");
  Serial.println("Leitura do sensor durante 2 segundos...");

  ACC = lerDistanciaEm2Segundos();
...
void executarADDK(uint16_t operando) {
  ACC = (uint16_t)(ACC + operando);

  Serial.print("EXEC   -> ADDK ");
  Serial.println(operando);
...
// =======================================================
// EXECUÇÃO DAS INSTRUÇÕES DE SAÍDA
// =======================================================
...
void executarBUZON() {
  ...
  Serial.print("EXEC   -> BUZON | pino ");
  Serial.println(PINO_BUZZER);
}

void executarBUZOFF() {
  digitalWrite(PINO_BUZZER, LOW);

  Serial.print("EXEC   -> BUZOFF | pino ");
  Serial.println(PINO_BUZZER);
}
...
void exibirEstadoSaidas() {
  ...
  if (digitalRead(PINO_BUZZER) == HIGH) {
    Serial.print("ON");
  } else {
    Serial.print("OFF");
  }
...
void exibirACC() {
  Serial.print("ACC    -> BIN = ");
  exibirPalavraBinaria16Bits(ACC);
  Serial.println();

  Serial.print("ACC    -> HEX = 0x");
  exibirHexadecimal4Digitos(ACC);
  Serial.println();
...
void exibirHexadecimal4Digitos(uint16_t valor) {
  uint8_t nibble3 = (valor >> 12) & 0x0F;
  uint8_t nibble2 = (valor >> 8) & 0x0F;
  uint8_t nibble1 = (valor >> 4) & 0x0F;
  uint8_t nibble0 = valor & 0x0F;

  Serial.print(nibble3, HEX);
  Serial.print(nibble2, HEX);
  Serial.print(nibble1, HEX);
  Serial.print(nibble0, HEX);
}